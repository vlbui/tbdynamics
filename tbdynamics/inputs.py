...
fixed_parameters = {
    ...
    "infect_death_rate_dict": {
        "smear_negative": 0.027588310343242016,
...
def get_birth_rate():
    return pd.read_csv(Path(DATA_PATH / 'camau_birth.csv'), index_col=0)['value']

def process_death_rate(age_strata: list):
    data = pd.read_csv(Path(DATA_PATH / 'data.csv'), usecols=['Age', 'Time', 'Population', 'Deaths'])
...
# Creating the new list
    agegroup_request = [[start, end - 1] for start, end in zip(age_strata, age_strata[1:] + [201])]
