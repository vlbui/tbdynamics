...
def get_tex_formatted_date(date):
    ...
    text_super = (
        special_cases[date_of_month] if date_of_month in special_cases else "th"
    )
    return (
        f'{date_of_month}\\textsuperscript{{{text_super}}}{date.strftime(" of %B %Y")}'
    )
...
class TexDoc(ABC):
    ...
    @abstractmethod
    def include_figure(
        self, caption: str, filename: str, section: str, subsection: str = ""
    ):
        pass
...
class ConcreteTexDoc:
    ...
    def emit_doc(
        ...
        content_sections = sorted(
            [s for s in self.content if s not in self.standard_sections]
        )
    ...
    def include_figure(
        ...
        self.add_line(
            f"\\includegraphics[width=\\textwidth]{{{filename}.jpg}}",
            section,
            subsection,
        )
    ...
    def include_table(
        ...
        col_format_str = " ".join(
            [
                f">{{\\raggedright\\arraybackslash}}p{{{width}cm}}"
        ...
        table_text = (
            table_text.replace("{tabular}", "{longtable}") if longtable else table_text
        )
